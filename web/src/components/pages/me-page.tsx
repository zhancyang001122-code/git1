  ...
  return (
    <AppShell activeNav="me" header={<PageHeader title="我的" />}>
      <div className="space-y-5 px-4 py-4">
        <section className="overflow-hidden rounded-feature bg-gradient-to-br from-brand-soft to-accent/10 p-5 shadow-card">
          <div className="flex items-center gap-4">
            <div className="relative size-16 overflow-hidden rounded-full bg-white shadow-floating">
              <Image
                fill
          ...
          <div className="mt-5 grid grid-cols-4 gap-2">
            {stats.map(({ href, icon: Icon, label, value }) => (
              <Link
                ...
                className="flex min-h-20 flex-col items-center justify-center rounded-control bg-white/75 px-1 text-center outline-none focus-visible:ring-2 focus-visible:ring-brand"
              >
        ...
        <section aria-labelledby="profile-preferences" className="space-y-3">
        ...
        >
          {entries.map(({ description, href, icon: Icon, label }) => (
            <Link
              key={label}
              href={href}
              className="flex min-h-16 items-center gap-3 border-b border-border px-4 outline-none last:border-b-0 hover:bg-brand-soft focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand"
            >
