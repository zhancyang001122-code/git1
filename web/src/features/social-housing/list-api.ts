...
const querySchema = z
  .object({
    ...
    sort: z.preprocess(
      emptyToUndefined,
      z
        .enum(["distance_asc", "price_asc", "price_desc", "published_desc"])
...
type SocialHousingRuntimeFactory = () => SocialHousingRuntime;
...
export function createSocialHousingLeadsHandler(
  runtimeFactory: SocialHousingRuntimeFactory = createSocialHousingRuntime,
) {
  return async function GET(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (city !== "杭州") {
        throw new AppError({
          code: "SOCIAL_HOUSING_UNSUPPORTED_CITY",
          message: "近期社交平台租房线索目前只覆盖杭州",
          status: 400,
          retryable: false,
        });
      }
      ...
      if (runtime.mode !== "supabase" || !runtime.service) {
        throw new AppError({
          code: "SOCIAL_HOUSING_NOT_CONFIGURED",
          message: "近期租房线索服务尚未配置",
          status: 503,
          retryable: true,
        });
      }
