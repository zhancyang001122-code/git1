...
- Supabase 提供跨 Vercel 实例的工具审计检索和四类站内阈值状态；检索不返回工具载荷。外部通知和值班升级尚未接入，不能称为完整企业告警平台。
- Chat、Feedback 和公开 Knowledge Search 已使用 Supabase 原子共享限流；客户端标识先经服务端 HMAC-SHA256，不保存原始 IP。低频单邮箱 OTP 仍保留已确认的简化边界。
