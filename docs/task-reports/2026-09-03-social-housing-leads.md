...
- `pnpm lint`：通过。
- `pnpm typecheck`：通过。
- `pnpm test`：145 个文件、621 个测试通过（批量发布收尾复验）。
...
- 最终得到 101 条准入候选、137 条自动拒绝和 1 条房源级重复，达到“100 条合格小红书近期线索”的采集目标。
...
- 2026-09-04 经作品集所有者明确批准，V2 的 101 条候选已通过可追溯批量审批导入；它们仍是“房态未经核验”的公开帖子线索，不是平台确认的实时库存。
