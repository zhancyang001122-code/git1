...
当前登录不是 Magic Link 或 OAuth callback，而是：
...
该流程不发生 Supabase 邮件链接跳转，因此 Redirect URLs 留空是与实现一致的最小配置。后续如果增加 Magic Link、OAuth 或密码重置，再按实际 callback 路径添加精确白名单，不能预先放宽为通配符。
...
因此，本报告只证明自定义网站域名与 Live 应用链路可用，不把邮件登录表述为已完成。
