...
桌面外部背景为 `#EDF0F5`，移动画布以 `#F4F6FA` 为基础并叠加极淡的蓝紫径向渐变。主内容底部至少留 `calc(72px + env(safe-area-inset-bottom))`。
...
| brand-600      | `#3158E6` | 按压态                         |
| violet-500     | `#7557F6` | AI 辅助渐变和强调              |
| page           | `#F4F6FA` | 小程序式页面背景               |
| surface        | `#FFFFFF` | 导航、Cell、卡片、弹层         |
| text-primary   | `#182033` | 主文字                         |
| text-secondary | `#5F6979` | 辅助说明                       |
| text-muted     | `#8791A2` | 占位和弱信息                   |
| border         | `#D6DEEB` | 细分割线与玻璃亮边             |
| success        | `#07C160` | 成功与授权完成，不作为品牌主色 |
| warning        | `#FA9D3B` | 风险提示                       |
| danger         | `#FA5151` | 错误和危险操作                 |

AI 大卡可以使用克制的 `linear-gradient(135deg, #EDF1FF, #F3EDFF)`。普通业务区域不使用大面积渐变或发光。

### 轻玻璃表面

- 磨砂玻璃只用于顶部/底部导航、弹层、AI 欢迎卡、公开进度和重要浮层；普通长列表不能为每一行叠加高强度模糊。
- 导航建议使用约 82% 白色、18px blur；关键面板约 84% 白色、14px blur，并保留可见细边框。
- 不允许低透明度灰字叠在复杂背景上；文字对比度优先于玻璃效果。
- 不支持 `backdrop-filter` 时必须自然降级为高不透明白色表面。
...
- 普通卡片使用统一的低对比蓝灰阴影，禁止页面自行创造不同发光效果。
- 只有底部弹层和悬浮反馈使用较明显阴影。

### 交互反馈

- 可点击卡片、按钮和胶囊统一使用 180ms 过渡；hover 显示低对比品牌亮边与轻阴影。
- pressed 使用 0.985 缩放和淡品牌 ring，不移动周围布局；触摸反馈不能只依赖 hover。
- 键盘 `focus-visible` 与 pressed 使用同一品牌状态体系，保持可见但不过度发光。
- `prefers-reduced-motion` 下取消缩放和点状位移动画。
...
- 背景为高不透明轻玻璃白色，顶部使用 1px 分割线，不使用大面积光晕。
...
- 进度区固定为 108px，只展示当前步骤；新步骤覆盖旧步骤，完整历史保留在调试/审计数据中而不撑长用户界面。
- 运行时使用三个低调动态点表达持续处理；完成、失败或超时后停止动画并显示最终状态。
- 无工具步骤时使用同尺寸“正在生成回答”状态，避免流式过程中布局跳动。
