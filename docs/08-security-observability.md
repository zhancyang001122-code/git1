...
当前实现提供进程内聚合耗时指标、JSON 结构化日志，以及由 Supabase `get_ai_ops_dashboard` RPC 生成的近 7 天持久化汇总。该 RPC 只向 `service_role` 授权，受保护的知识运营页展示 Token、工具失败率、反馈、评测和知识库存，不读取原始对话；没有模型价格配置时不估算人民币成本。跨实例日志检索、趋势图和告警仍需接入托管可观测平台，不能把现有汇总描述成完整生产监控。
