...
当前实现提供进程内聚合耗时指标、JSON 结构化日志，以及由 Supabase `get_ai_ops_dashboard` 和 `get_rag_ops_trend` RPC 生成的持久化汇总。RPC 只向 `service_role` 授权，受保护的知识运营页展示 Token、终态工具失败率、反馈、评测、知识库存及按北京时间补零的 RAG 日趋势，不读取原始对话；没有模型价格配置时不估算人民币成本。跨实例日志检索和告警仍需接入托管可观测平台，不能把现有汇总描述成完整生产监控。
