...
`/houses` 的近期线索页签通过独立的 `SocialHousingSearchService` 查询 Supabase，不与 2024 历史房源表混写。当前阶段该能力没有注册为 Agent 工具，因此线索列表与详情不展示无法取证的“问小智”操作；后续只有在工具契约、引用卡和固定评测齐备后才可接入 Agent。

采集原文只在本机暂存。进入 Supabase 的内容只包含脱敏摘要、租金上下限、结构化位置、坐标、审核状态、哈希和 canonical 原帖链接。模型负责分类与字段抽取，展示资格仍由确定性规则和人工审核决定。
